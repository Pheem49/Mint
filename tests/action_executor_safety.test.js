...
jest.mock('../dist/src/Automation_Layer/file_operations', () => ({
...
jest.mock('../dist/src/Automation_Layer/open_app', () => ({
    openApp: jest.fn()
}));

jest.mock('../dist/src/Automation_Layer/open_website', () => ({
    openWebsite: jest.fn(),
    openSearch: jest.fn()
}));

jest.mock('../dist/src/Automation_Layer/browser_automation', () => ({
    performWebAutomation: jest.fn(async () => 'done')
}));

jest.mock('../dist/src/AI_Brain/knowledge_base', () => ({
...
jest.mock('../dist/src/System/granular_automation', () => ({
    mouseMove: jest.fn(),
    mouseClick: jest.fn(),
    typeText: jest.fn(),
    keyTap: jest.fn()
...
jest.mock('../dist/src/System/system_automation', () => ({
    shutdown: jest.fn(),
    restart: jest.fn(),
    sleep: jest.fn(),
    setVolume: jest.fn(),
...
describe('action_executor safety integration', () => {
    test('blocks dangerous delete actions unless explicitly allowed', async () => {
        const { executeAction } = require('../dist/src/System/action_executor');

...
export function buildBubblewrapArgs(command: string, options: any = {}): string[] {
...
export function buildMacSandboxProfile(options: any = {}): string {
    const cwd = path.resolve(options.cwd || process.cwd())
    const readRoots = uniqueExistingRoots(safetyManager.getAllowedRoots('read'))
    const writeRoots = uniqueExistingRoots(safetyManager.getAllowedRoots('write'))
...
function execFilePromise(command: string, args: string[], options: any = {}): Promise<{ stdout: string; stderr: string }> {
...
export async function runShell(command: string, options: any = {}) {
